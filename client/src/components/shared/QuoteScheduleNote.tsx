...
/**
 * Customer-facing job SCHEDULE line: how many movers, for how long, and the
 * overtime rate beyond that. Scheduled time is derived from the job scope
 * (items + distance) via getJobSchedule — never from dividing the price.
 * Rendered on the estimate, quote status, invoice, PDF, and admin quote screen
 * so the customer always sees what their price covers.
 */
export function QuoteScheduleNote({
  ...
  const hrs = s.scheduledHours % 1 === 0 ? `${s.scheduledHours}` : s.scheduledHours.toFixed(1);
      ...
      <div className="flex items-start gap-2">
        ...
        <div className="text-[11px] leading-relaxed text-gray-600">
          <p data-testid="text-scheduled-time">
            ...
            {movers} × ~{hrs} hour{hrs === "1" ? "" : "s"} on site.
          </p>
          <p data-testid="text-overtime-rate" className="text-gray-500">
            ...
            (billed in {s.blockMinutes}-minute blocks).
          </p>
        </div>
      </div>
    </div>
