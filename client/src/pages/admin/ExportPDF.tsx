...
function DetailPanel({ q }: { q: any }) {
 ...
 <span className="font-mono text-lg font-black text-[#0A0A0A]">{q.referenceNo}</span>
 ...
 {items.length === 0 ? (
 <p className="text-xs text-black/45 italic">No line items.</p>
 ) : (
 <div className="border border-gray-100 overflow-hidden">
 <table className="w-full text-xs">
 <thead>
 <tr className="bg-gray-50 text-black/55">
 ...
 <td className="px-3 py-2 text-right text-black/65">{it.quantity}</td>
 <td className="px-3 py-2 text-right text-black/65">{money(it.unitPrice)}</td>
 ...
 <div className={`flex items-center justify-between text-xs ${q.finalPaidAt ? "text-emerald-600" : "text-black/45"}`}>
 <div className="flex items-center gap-1">
 {q.finalPaidAt ? <CheckCircle2 className="w-3.5 h-3.5" /> : <div className="w-3.5 h-3.5 rounded-full border border-black/20" />}
 <span>Final 50%</span>
 </div>
 <div className="text-right">
 ...
 {arrived ? (
 <div className="bg-[#EBE9E2] border border-blue-100 p-4">
 ...
 <p className="text-xs text-[#0A0A0A] mt-0.5">{format(new Date(arrived.createdAt), "EEE, d MMM yyyy")}</p>
 <p className="text-[10px] text-blue-300 mt-1 font-mono">
 {Number(arrived.gpsLat).toFixed(5)}, {Number(arrived.gpsLng).toFixed(5)}
 </p>
 </div>
 ) : (
 <div className="border-2 border-dashed border-black/12 p-4 flex flex-col items-center justify-center text-gray-300 text-xs">
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
 return (
 <div>
 <p className="text-[10px] font-bold uppercase tracking-widest text-black/45 mb-2">{title}</p>
 {children}
 </div>
 );
}
...
function InfoRow({ label, value }: { label: string; value: string }) {
 ...
 <span className="text-black/45 w-20 shrink-0">{label}</span>
...
function FinLine({ label, value, red }: { label: string; value: string; red?: boolean }) {
 return (
 <div className={`flex justify-between text-xs ${red ? "text-red-500" : "text-black/65"}`}>
...
export default function ExportPDF() {
 ...
 {/* ── Toolbar ── */}
 <div className="bg-white border-b border-black/12 px-4 py-4 shrink-0 z-20 ">
 <div className="flex items-center justify-between gap-3">
 <div className="flex items-center gap-3 min-w-0">
 {/* Mobile: back from detail to list */}
 {mobileView === "detail" ? (
 <button onClick={() => setMobileView("list")}
 className="inline-flex items-center gap-1.5 h-8 px-3 rounded-lg text-black/65 hover:bg-[#EBE9E2] text-sm font-medium transition-colors shrink-0 sm:hidden">
 <ArrowLeft className="w-4 h-4" /> Back
 </button>
 ) : (
 <Link href="/admin" className="inline-flex items-center gap-1.5 h-8 px-3 rounded-lg text-black/65 hover:bg-[#EBE9E2] text-sm font-medium transition-colors shrink-0">
 <ArrowLeft className="w-4 h-4" />
 <span className="hidden sm:inline">Dashboard</span>
 </Link>
 )}
 <div className="min-w-0 border-l border-black/12 pl-3">
 <p className="text-sm font-semibold text-[#0A0A0A] truncate">Audit Report</p>
 ...
 {hasFilter && <span className="text-black/45"> · filtered</span>}
 </p>
 </div>
 </div>
 <div className="flex items-center gap-2 shrink-0">
 <button onClick={() => setShowFilters(f => !f)}
 className={`inline-flex items-center gap-2 h-9 px-4 rounded-lg text-sm font-medium transition-colors ${showFilters || hasFilter ? "bg-[#EBE9E2] text-[#0A0A0A]" : "bg-white border border-black/12 text-black/70 hover:bg-white"}`}
 ...
 className="inline-flex items-center px-3 py-1.5 rounded-md bg-[#EBE9E2] hover:bg-[#EBE9E2] text-xs font-medium text-black/70 transition-colors">{l}</button>
 ))}
 </div>
 <div className="grid grid-cols-2 sm:flex sm:flex-wrap gap-3 sm:gap-4">
 <div className="flex-1 min-w-[140px]">
 <p className="text-xs font-medium text-black/55 mb-1.5">From Date</p>
 <input type="date" value={dateFrom} onChange={e => setDateFrom(e.target.value)}
 className="h-9 w-full px-3 border border-black/20 rounded-lg text-sm bg-white focus:outline-none focus:ring-2 focus:ring-[#0A0A0A] focus:border-[#0A0A0A] transition-colors" data-testid="input-filter-from" />
 </div>
 <div className="flex-1 min-w-[140px]">
 <p className="text-xs font-medium text-black/55 mb-1.5">To Date</p>
 <input type="date" value={dateTo} onChange={e => setDateTo(e.target.value)}
 className="h-9 w-full px-3 border border-black/20 rounded-lg text-sm bg-white focus:outline-none focus:ring-2 focus:ring-[#0A0A0A] focus:border-[#0A0A0A] transition-colors" data-testid="input-filter-to" />
 </div>
 <div className="col-span-2 sm:col-span-1 flex flex-col justify-end">
 <div className="flex p-1 rounded-lg border border-black/12 bg-[#EBE9E2] h-9">
 {([["all","All"],["final_paid","Paid"],["closed","Unpaid"]] as const).map(([v,l]) => (
 <button key={v} onClick={() => setStatusFilter(v)}
 className={`flex-1 sm:px-4 text-xs font-semibold rounded-md transition-all ${statusFilter === v ? "bg-white text-[#0A0A0A] border border-black/12" : "text-black/55 hover:text-black/70"}`}>{l}</button>
 ))}
 </div>
 </div>
 </div>
 {hasFilter && (
 <button onClick={clear} className="inline-flex items-center gap-1.5 text-xs text-red-600 font-medium hover:text-red-700 transition-colors mt-2">
 ...
 <div className="grid grid-cols-4 gap-px bg-[#EBE9E2] shrink-0 sm:hidden">
 <div className="bg-white px-3 py-2 text-center">
 <p className="text-[10px] text-black/55 font-medium">Revenue</p>
 <p className="text-sm font-bold text-[#0A0A0A] truncate mt-0.5">{money(rev)}</p>
 </div>
 <div className="bg-white px-3 py-2 text-center">
 <p className="text-[10px] text-black/55 font-medium">Paid</p>
 <p className="text-sm font-bold text-[#0A0A0A] mt-0.5">{paid}/{filteredJobs.length}</p>
 </div>
 <div className="bg-white px-3 py-2 text-center">
 <p className="text-[10px] text-black/55 font-medium">Deposits</p>
 <p className="text-sm font-bold text-[#0A0A0A] truncate mt-0.5">{money(dep)}</p>
 </div>
 <div className="bg-white px-3 py-2 text-center">
 <p className="text-[10px] text-black/55 font-medium">Finals</p>
 <p className="text-sm font-bold text-[#0A0A0A] truncate mt-0.5">{money(fin)}</p>
 </div>
 </div>
 ...
 sm:w-80 xl:w-96 sm:border-r border-black/12 sm:shrink-0
 ${mobileView === "detail" ? "hidden sm:flex" : "flex w-full"}`}>

 {/* Stats strip — desktop only */}
 <div className="hidden sm:grid grid-cols-2 gap-px bg-[#EBE9E2] shrink-0 border-b border-black/12">
 <div className="bg-white px-4 py-3">
 <p className="text-xs text-black/55 font-medium mb-1">Revenue</p>
 <p className="text-lg font-bold text-[#0A0A0A]">{money(rev)}</p>
 </div>
 <div className="bg-white px-4 py-3">
 <p className="text-xs text-black/55 font-medium mb-1">Fully Paid</p>
 <p className="text-lg font-bold text-[#0A0A0A]">{paid} / {filteredJobs.length}</p>
 </div>
 <div className="bg-white px-4 py-3 border-t border-black/8">
 <p className="text-xs text-black/55 font-medium mb-1">Deposits</p>
 <p className="text-sm font-semibold text-black/70">{money(dep)}</p>
 </div>
 <div className="bg-white px-4 py-3 border-t border-black/8 border-l border-black/8">
 <p className="text-xs text-black/55 font-medium mb-1">Finals</p>
 <p className="text-sm font-semibold text-black/70">{money(fin)}</p>
 </div>
 </div>

 {/* Search */}
 <div className="p-3 border-b border-black/12 shrink-0 bg-white/50">
 ...
 className="w-full pl-9 pr-3 h-9 text-sm border border-black/20 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-[#0A0A0A] focus:border-[#0A0A0A] transition-colors"
 data-testid="input-search-jobs"
 />
 {search && (
 <button onClick={() => setSearch("")} className="absolute right-2.5 top-1/2 -translate-y-1/2 text-black/45 hover:text-black/65">
 <X className="w-4 h-4" />
 </button>
 )}
 </div>
 ...
 {jobs.length === 0 ? (
 <div className="flex flex-col items-center justify-center h-full text-black/45 text-sm py-12 px-4">
 <FileText className="w-8 h-8 mb-3 text-zinc-300" />
 <p className="font-medium text-black/65">No jobs found</p>
 {(hasFilter || search) && (
 <button onClick={() => { clear(); setSearch(""); }} className="mt-2 text-[#0A0A0A] font-medium hover:underline">Clear filters</button>
 )}
 </div>
 ) : (
 ...
 className={`w-full text-left p-4 transition-colors ${isSelected ? "bg-[#EBE9E2]" : "hover:bg-white"}`}
 data-testid={`row-job-${q.id}`}
 >
 <div className="flex items-start justify-between gap-2 mb-1.5">
 <span className="font-mono text-xs font-semibold text-black/55">{q.referenceNo}</span>
 <span className={`inline-flex items-center rounded-md px-1.5 py-0.5 text-[10px] font-semibold ${statusCls(q.status)}`}>
 {statusLabel(q.status)}
 </span>
 </div>
 <p className="text-sm font-semibold text-[#0A0A0A] truncate mb-1">{q.customer?.name || "—"}</p>
 ...
 <p className="text-sm font-bold text-[#0A0A0A] shrink-0">{money(q.total)}</p>
 </div>
 {q.assignedStaff?.name && (
 <p className="text-xs text-black/45 mt-1.5 truncate flex items-center gap-1.5">
 <CheckCircle2 className="w-3.5 h-3.5" /> {q.assignedStaff.name}
 </p>
 )}
 </button>
 );
 ...
 {/* List footer */}
 <div className="border-t border-black/12 bg-white px-4 py-3 shrink-0">
 <div className="flex justify-between text-xs font-medium text-black/65">
 <span>{jobs.length} job{jobs.length !== 1 ? "s" : ""}{search ? ` matching "${search}"` : ""}</span>
 <span className="font-bold text-[#0A0A0A]">{money(jobs.reduce((s: number, q: any) => s + Number(q.total || 0), 0))}</span>
 </div>
 </div>
 </div>
 ...
 <p className="text-sm font-medium text-black/65 text-center">Select a job to view its details</p>
 <p className="text-xs text-black/45 mt-1">{jobs.length} job{jobs.length !== 1 ? "s" : ""} in the list</p>
 </div>
 )}
 </div>
 </div>
