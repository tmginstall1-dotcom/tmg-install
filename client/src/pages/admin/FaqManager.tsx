...
const CATEGORY_COLORS: Record<string, string> = {
 general: "bg-[#EBE9E2] text-black/70",
 services: "bg-[#EBE9E2] text-[#0A0A0A]",
 pricing: "bg-emerald-50 text-emerald-700",
 booking: "bg-[#EBE9E2] text-[#0A0A0A]",
...
function FaqModal({
 ...
 <h2 className="text-base font-semibold text-[#0A0A0A]">{entry?.id ? "Edit FAQ" : "Add FAQ"}</h2>
 ...
 <div>
 <Label className="text-xs text-black/55 uppercase tracking-wide font-semibold mb-1.5 block">Question</Label>
 <Input
 ...
 <div>
 <Label className="text-xs text-black/55 uppercase tracking-wide font-semibold mb-1.5 block">Answer</Label>
 <Textarea
 ...
 <div>
 <Label className="text-xs text-black/55 uppercase tracking-wide font-semibold mb-1.5 block">Category</Label>
 <select
 data-testid="select-faq-category"
 value={form.category}
 onChange={e => setForm(f => ({ ...f, category: e.target.value }))}
 className="w-full h-9 px-3 border border-black/20 rounded-lg text-sm focus:ring-2 focus:ring-[#0A0A0A] focus:border-[#0A0A0A] outline-none bg-white"
 >
...
function CannedModal({
 ...
 <div className="bg-white rounded-none border border-black/12 w-full max-w-lg flex flex-col">
 <div className="flex items-center justify-between px-6 py-4 border-b border-black/8">
 <h2 className="text-base font-semibold text-[#0A0A0A]">{reply?.id ? "Edit Canned Reply" : "Add Canned Reply"}</h2>
 <button onClick={onClose} className="text-black/45 hover:text-black/65">
 <X className="w-4 h-4" />
 </button>
 </div>
 ...
 <div className="grid grid-cols-2 gap-3">
 <div>
 <Label className="text-xs text-black/55 uppercase tracking-wide font-semibold mb-1.5 block">Shortcut</Label>
 <Input
 ...
 placeholder="/quote"
 className="h-9 text-sm border-black/20 font-mono"
 />
 <p className="text-[11px] text-black/45 mt-1">Must start with /</p>
 </div>
 <div>
 <Label className="text-xs text-black/55 uppercase tracking-wide font-semibold mb-1.5 block">Title</Label>
 <Input
 ...
 <div>
 <Label className="text-xs text-black/55 uppercase tracking-wide font-semibold mb-1.5 block">Message Body</Label>
 <Textarea
 ...
 className="text-sm border-black/20 resize-none font-mono"
 />
 <p className="text-[11px] text-black/45 mt-1">Use \n for new lines. Plain text only.</p>
 </div>
 <div className="flex items-center gap-3">
 <button
...
function FaqTab() {
 ...
 <div>
 <h2 className="text-sm font-semibold text-[#0A0A0A]">FAQ Entries</h2>
 <p className="text-xs text-black/55 mt-0.5">{activeFaqs} active · {entries.length} total</p>
 </div>
 <Button
 ...
 {/* Filters */}
 <div className="px-5 py-3 border-b border-black/8 flex items-center gap-3 flex-wrap">
 <div className="relative flex-1 min-w-[180px]">
 <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-black/45" />
 <Input
 data-testid="input-faq-search"
 value={search}
 onChange={e => setSearch(e.target.value)}
 placeholder="Search questions..."
 className="h-9 pl-9 text-sm border-black/20"
 />
 </div>
 <select
 ...
 className="h-9 px-3 border border-black/20 rounded-lg text-sm focus:ring-2 focus:ring-[#0A0A0A] outline-none bg-white"
 >
 ...
 <p className="text-sm text-black/55">No FAQ entries found</p>
 ...
 {filtered.map(entry => (
 <div key={entry.id} data-testid={`faq-row-${entry.id}`} className={`px-5 py-3.5 hover:bg-white transition-colors ${!entry.active ? "opacity-50" : ""}`}>
 ...
 <span className="text-sm font-medium text-[#0A0A0A]">{entry.question}</span>
 <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-md ${CATEGORY_COLORS[entry.category] || "bg-[#EBE9E2] text-black/70"}`}>
 {FAQ_CATEGORIES.find(c => c.value === entry.category)?.label || entry.category}
 </span>
 {!entry.active && (
 <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-md bg-[#EBE9E2] text-black/55">Inactive</span>
 )}
 </div>
 {expandedId === entry.id ? (
 <p className="text-sm text-black/65 mt-2 whitespace-pre-wrap leading-relaxed">{entry.answer}</p>
 ) : (
 <p className="text-xs text-black/45 mt-0.5 truncate">{entry.answer}</p>
 )}
 </button>
 </div>
...
function CannedRepliesTab() {
 ...
 <div>
 <h2 className="text-sm font-semibold text-[#0A0A0A]">Canned Replies</h2>
 <p className="text-xs text-black/55 mt-0.5">Quick reply templates for manual admin responses</p>
 </div>
 <Button
 ...
 <p className="text-sm text-black/55">No canned replies yet</p>
 </div>
 ) : (
 <div className="divide-y divide-black/8">
 {replies.map(reply => (
 <div key={reply.id} data-testid={`canned-row-${reply.id}`} className={`px-5 py-3.5 hover:bg-white transition-colors ${!reply.active ? "opacity-50" : ""}`}>
 <div className="flex items-start gap-3">
 <div className="flex-1 min-w-0">
 <div className="flex items-center gap-2 flex-wrap mb-1">
 <code className="text-xs font-mono bg-[#EBE9E2] text-black/70 px-1.5 py-0.5 rounded-md">{reply.shortcut}</code>
 <span className="text-sm font-medium text-[#0A0A0A]">{reply.title}</span>
 {!reply.active && (
 <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-md bg-[#EBE9E2] text-black/55">Inactive</span>
 )}
 </div>
 <p className="text-xs text-black/55 whitespace-pre-wrap line-clamp-2">{reply.body}</p>
 </div>
 <div className="flex items-center gap-1 shrink-0 ml-2">
 <button
...
function CorrectionModal({
 ...
 <button onClick={onClose} className="text-black/45 hover:text-black/65"><X className="w-4 h-4" /></button>
 </div>
 <div className="p-6 space-y-4 overflow-y-auto">
 <div className="bg-[#EBE9E2] border border-violet-100 rounded-lg px-4 py-3 text-xs text-violet-800">
 ...
 <div>
 <Label className="text-xs text-black/55 uppercase tracking-wide font-semibold mb-1.5 block">
 ...
 className="h-9 text-sm border-black/20"
 />
 <p className="text-[11px] text-black/45 mt-1">The phrase or item name that gets misidentified</p>
 </div>
 <div>
 <Label className="text-xs text-black/55 uppercase tracking-wide font-semibold mb-1.5 block">
 ...
 className="h-9 text-sm border-black/20 font-mono text-xs"
 />
 <p className="text-[11px] text-black/45 mt-1">Leave blank to use corrected name as the lookup key</p>
 </div>
 <div>
 <Label className="text-xs text-black/55 uppercase tracking-wide font-semibold mb-1.5 block">Admin notes (optional)</Label>
 <Input
 ...
 onClick={() => onSave(form)}
 className="h-9 px-4 text-sm bg-[#0A0A0A] hover:bg-violet-700 text-white"
 >
...
function BotTrainingTab() {
 ...
 <div className="bg-white border border-black/12 rounded-none px-4 py-3">
 <p className="text-xs text-black/55">Total Rules</p>
 <p className="text-2xl font-bold text-[#0A0A0A] mt-0.5">{corrections.length}</p>
 <p className="text-[11px] text-black/45">{activeCount} active</p>
 </div>
 <div className="bg-white border border-violet-100 rounded-none px-4 py-3">
 <p className="text-xs text-[#0A0A0A] flex items-center gap-1"><Brain className="w-3 h-3" /> Auto-Learned</p>
 <p className="text-2xl font-bold text-[#0A0A0A] mt-0.5">{autoCount}</p>
 <p className="text-[11px] text-black/45">discovered from live chats</p>
 </div>
 <div className="bg-white border border-blue-100 rounded-none px-4 py-3">
 <p className="text-xs text-[#0A0A0A]">Manually Added</p>
 <p className="text-2xl font-bold text-[#0A0A0A] mt-0.5">{manualCount}</p>
 <p className="text-[11px] text-black/45">added by admin</p>
 </div>
 </div>
 ...
 filter === f
 ? "bg-violet-100 text-[#0A0A0A]"
 : "bg-[#EBE9E2] text-black/55 hover:bg-[#EBE9E2]"
 }`}
 >
 ...
 className="h-8 px-3 text-xs bg-[#0A0A0A] hover:bg-violet-700 text-white gap-1.5"
 >
 ...
 className={`px-5 py-3.5 hover:bg-white transition-colors ${!c.active ? "opacity-50" : ""}`}
 >
 <div className="flex items-start gap-3">
 <div className="flex-1 min-w-0">
 <div className="flex items-center gap-2 flex-wrap mb-0.5">
 {c.autoLearned && (
 <span className="inline-flex items-center gap-1 text-[10px] font-semibold px-1.5 py-0.5 rounded-md bg-violet-100 text-[#0A0A0A]">
 ...
 <span className="inline-flex items-center gap-1 text-xs font-mono bg-[#EBE9E2] text-violet-800 border border-violet-200 px-1.5 py-0.5 rounded">
 <CheckCircle className="w-3 h-3" />
 {c.correctedName}
 </span>
 {!c.active && <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-md bg-[#EBE9E2] text-black/55">Inactive</span>}
 </div>
 {c.catalogItemName && (
 <p className="text-[11px] text-black/45 mt-0.5">
 Catalog: <code className="bg-[#EBE9E2] px-1 rounded text-[10px]">{c.catalogItemName}</code>
 </p>
 )}
 {c.notes && (
 <p className="text-xs text-black/55 mt-0.5 italic">{c.notes}</p>
 )}
 </div>
...
export default function FaqManager() {
 ...
 return (
 <div className="min-h-screen pt-14 pb-16 lg:pl-56 bg-[#F5F4F0]">
 {/* Page header */}
 <div className="bg-white border-b border-black/12">
 <div className="max-w-5xl mx-auto px-6 py-5">
 <p className="text-xs text-black/45 mb-1">Chatbot</p>
 <div className="flex items-center justify-between gap-4">
 <div>
 <h1 className="text-2xl font-semibold text-[#0A0A0A]">FAQ Manager</h1>
 <p className="text-sm text-black/55 mt-0.5">Manage knowledge used by the WhatsApp bot to answer customer questions</p>
 </div>
 </div>
 {/* Tabs */}
 <div className="flex gap-1 mt-4 border-b border-black/12 -mb-5">
 <button
 data-testid="tab-faq"
 onClick={() => setTab("faq")}
 className={`pb-4 px-1 text-sm font-medium border-b-2 transition-colors ${
 tab === "faq"
 ? "border-blue-600 text-[#0A0A0A]"
 ...
 {/* Info banner */}
 <div className={`border rounded-none px-5 py-4 flex items-start gap-3 ${
 tab === "training"
 ? "bg-[#EBE9E2] border-violet-100"
 : "bg-[#EBE9E2] border-blue-100"
 }`}>
 {tab === "training"
 ...
 <p className={`text-xs mt-0.5 ${tab === "training" ? "text-[#0A0A0A]" : "text-[#0A0A0A]"}`}>
 {tab === "faq"
