...
function eventColor(event: string) {
 ...
 case "cta_click": return "bg-blue-50 text-[#0A0A0A]";
 case "wizard_start": return "bg-[#EBE9E2] text-[#0A0A0A]";
...
function OperationsTab({ days }: { days: number }) {
 ...
 <div className="bg-white border border-zinc-200 rounded-none p-6 text-center">
 ...
 {/* Staff summary table */}
 {staffAttendance.length > 0 && (
 <div className="bg-white border border-zinc-200 rounded-none overflow-hidden mt-4">
...
function WebsiteTab({ days }: { days: number }) {
 ...
 <div key={label} className="bg-white border border-zinc-200 rounded-none p-5">
 <div className="flex items-start justify-between gap-2 mb-3">
 <div className="w-8 h-8 rounded-lg bg-zinc-50 flex items-center justify-center shrink-0">
 ...
 className={`px-2.5 py-1 text-[10px] font-semibold rounded-sm transition-colors ${mapView === v ? "bg-white text-zinc-900 " : "text-zinc-500 hover:text-zinc-700"}`}>
 {v.toUpperCase()}
 </button>
 ))}
 </div>
 ...
 {hoveredPin && (
 <div className="absolute pointer-events-none z-10 bg-black text-white text-[11px] font-semibold px-2 py-1 whitespace-nowrap "
 ...
 <td className="px-4 py-3 hidden lg:table-cell text-[#0A0A0A]">{evt.utmCampaign && <span className="truncate max-w-[100px] inline-block">📣 {evt.utmCampaign}</span>}</td>
...
function PnLTab() {
 ...
 <div className="bg-white border border-slate-200 rounded-none px-4 py-3 space-y-3">
 ...
 className="h-8 px-2 text-xs border border-zinc-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#0A0A0A]"
 />
 ...
 sub="Full salary + hourly + OT + meal (matches payslip)" icon={Users} color="text-red-400" />
 ...
 {pendingExpenses > 0 && (
 <div className="flex items-center gap-3 bg-amber-50 border border-amber-200 rounded-none px-5 py-3.5">
 ...
 <div className="flex items-center justify-between rounded-lg px-3 py-2 bg-[#EBE9E2]">
 <span className="text-xs font-medium text-zinc-700">GoGoVan Net Earnings</span>
 <span className="text-sm font-bold text-[#0A0A0A]">{fmtSGD(ggvRevenue)}</span>
 </div>
 ...
 <span className={`text-sm font-bold ${profitable ? "text-[#0A0A0A]" : "text-red-600"}`}>{profitMargin}%</span>
 </div>
 ...
 Staff Salary = full monthly salary for each month employed, plus actual hourly + overtime + meal allowance from clock in/out records — matches each staff's payslip. Hourly staff = hours logged × hourly rate + overtime + meal allowance.
...
export default function Analytics() {
 ...
 tab === key ? "bg-white text-zinc-900 border border-zinc-200" : "text-zinc-500 hover:text-zinc-700"
 }`}>
 ...
 activeDays === opt.value ? "bg-white text-zinc-900 border border-zinc-200" : "text-zinc-500 hover:text-zinc-700"
 }`}>
 {opt.label}
 </button>
 ))}
