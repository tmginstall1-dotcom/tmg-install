...
**Van-pickup label + auto-calc limits:** the off-site reason labels the clock-in "the van pickup point (Woodlands St 31)" whenever within VAN_MATCH_RADIUS_M (500m) of VAN_PICKUP, regardless of flags — so the "rode out with the driver" case reads clearly even when reverseGeocodeSg is vague. Two hard limits make auto-duration impossible on some days (admin must type minutes): (1) suggestedMinutes needs a continuous GPS TRACK between clock-in/out — attendance rows carry only single clock-in/out points, and per-staff GPS history often starts mid-month, so earlier days compute 0; (2) firstJobGeo needs a first-job location that day. Missing either → no computed van→job time. This is DATA, not a code bug.

**GGV jobs as the day's first-job anchor:** the first-job source is NOT only quotes assigned to the staff (getQuotesAssignedTo). GoGoVan jobs live in ggv_jobs and only a minority are mirrored into an assigned quote — the historical bulk carry quote_id=null (no assignment at all). So when a day has no assigned-quote first job AND the staff is GGV crew, the review falls back to storage.getGGVJobs(day) via the pure helper `pickFirstGgvJobForStaff` (server/lib/ggv-first-job.ts) — earliest by time_start with a usable address. **isGgvCrew** = has any sourceChannel==='gogovan' quote assigned OR carries mustClockInAtFirstJob / canClockInAtVanPickup flag. **Deliberate deviation from architect:** the helper INCLUDES unassigned GGV rows (assignedStaffId & assignedTeamId both null), not just rows assigned to the staff/team. Reason: all historical GGV rows are unassigned, and the GGV crew is a single team that rides out together, so dropping them would leave every historical GGV day showing "no scheduled job" — the exact bug being fixed. Rows assigned to ANOTHER team are still excluded.
