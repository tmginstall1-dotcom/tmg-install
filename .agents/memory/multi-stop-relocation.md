...
**Display surfaces that must stay in lockstep** (grouped stops + per-item route tags): `client/src/pages/Invoice.tsx`, `client/src/lib/invoicePdf.ts`, `client/src/pages/admin/QuoteDetail.tsx` (view + edit), `client/src/pages/customer/QuoteStatus.tsx`, `client/src/pages/staff/JobDetail.tsx` (staff mobile job view). `server/routes.ts` `buildInvoicePayload` already exposes `stops[]` + item `fromStopId`/`toStopId`; the plain `GET /api/quotes/:id` (used by the `useQuote` hook for both QuoteStatus and staff JobDetail) returns the full quote incl. `stops` and item stop ids.
