...
- OT split mirrors the payslip generator: net hours = (clockOut − clockIn − deductionMinutes − closed unpaid breaks); regular capped at 8h/day; remainder is OT; OT rate falls back to 1.5× hourly when no explicit overtime rate is set. The P&L `allAttLogs` select MUST include `breaks` and subtract them, or P&L cost + meal allowance drift from real payslip pay.
- Meal (dinner) allowance mirrors the payslip via the shared `mealAllowanceForDay()` in `shared/pricing.ts` (single source of truth — do NOT re-hardcode the threshold). Current policy from 1 Jul 2026: $8 on any day with >= 2h OT, no lunch allowance; before that, legacy $8 when OT > 3h. The P&L calc reproduces payslip gross to the cent when replayed against real attendance.
