...
2. WARDROBE / CLOSET: Tall cabinet (typically 180–240cm) with HINGED or SLIDING DOORS. Contains hanging space for clothes. May or may not have drawers at the bottom. Names: "wardrobe", "IKEA PAX wardrobe", "2-door wardrobe", etc.
   ...
   2a. WARDROBE PANEL COUNTING — CRITICAL (read carefully, this is the single most common pricing mistake):
       A sliding-door wardrobe is ONE single piece of furniture, no matter how wide it is. Count the number of full-height panels (each panel = roughly 60–90cm wide, runs floor-to-ceiling or close to it) along the front and use the matching SKU:
          • 2 panels  → "Sliding Door Wardrobe (2-door)"
   ...
   2b. IKEA PAX WARDROBE — VERY COMMON, and it is PRICED PER FRAME (not as a single sliding wardrobe). Read this carefully:
       ...
       • Set quantity = the NUMBER OF FRAMES / BAYS (each bay ≈ 50–100cm wide vertical section). For an L-shape / CORNER PAX, count the corner section as ONE extra frame and add the frames on each side (e.g. two bays + corner + two bays = quantity 5 frames total).
       • A SINGLE PAX run frequently MIXES door mechanisms — e.g. some bays are open (no doors, used for hanging clothes or visible drawers/shelves), some bays have hinged doors, some have sliding/mirror doors. Do NOT collapse a mixed run into one mechanism. Instead, count the frames of EACH mechanism separately and return ONE line item PER mechanism that is present, each with its own frame count. Count the corner section as one frame and assign it to the mechanism of ITS OWN door (open if the corner has no door, hinged if it has a hinged door, sliding if it slides). Example for an L-shape with one open hanging bay + two hinged-door bays + a hinged-door corner + one hinged-door bay + one open drawer/shelf bay: return {"name":"IKEA PAX Wardrobe (per frame, hinged doors)","quantity":4} (3 hinged side bays + the hinged corner) AND {"name":"IKEA PAX Wardrobe (per frame, no doors)","quantity":2} (the two open bays). The frame counts across all the PAX line items must add up to the TOTAL number of frames including the corner.
       • Only use ONE line item when the whole run genuinely uses a SINGLE door mechanism. A bay where you can see hanging clothes, open shelves, or exposed drawers with NO door in front of it is an OPEN frame ("no doors"), even if the bays next to it have doors.
       • A PAX (even a wide one, even with mirror doors, even an L-shape corner, even with a mix of open and doored bays) must map to the per-frame PAX items above — do NOT map it to the generic "Sliding Door Wardrobe" SKUs.
    ...
    try {
      ...
      const { customerUpdates, quoteUpdates, items } = z.object({
        ...
        quoteUpdates: z.object({
          ...
          secondDayCrewSize: z.coerce.number().int().min(1).max(20).optional(),
...
- WALL-HUNG / WALK-IN / BUILT-IN WARDROBE (very important — these are priced PER HOLE, not per piece):
  • ONLY use the per-hole "Walk-in / Built-in Wardrobe (per hole)" classification when the system is CLEARLY drilled into a wall — i.e. you can see EXPOSED vertical metal standards / wood uprights ANCHORED into a brick or drywall surface, with open hanging rails, open wire shelves, or basket racks suspended off the wall and NO enclosed carcass boxes and NO full-height doors. Examples: Elfa, IKEA Algot, IKEA Boaxel, custom open-rail carpentry walk-in wardrobe inside a room.
  • CRITICAL — IKEA PAX is NOT per-hole. A PAX is FREESTANDING furniture (modular enclosed box carcasses standing on their own base, tall flat bays side by side, hinged or sliding full-height doors, often a mix of solid + mirror doors, bays can meet at a 90° corner) and is priced PER FRAME — EVEN an L-shape / corner run, EVEN when some bays are open for hanging clothes or drawers, EVEN when pushed against or fixed to a wall for anti-tip safety. When you recognise a PAX, map it with the per-frame PAX rule in section 2b below — NEVER to "Walk-in / Built-in Wardrobe (per hole)".
  • NOTE: "not per-hole" does NOT automatically mean PAX. Only classify as PAX when the modular PAX tell-tales above are visible. A genuine non-PAX enclosed wardrobe with sliding or hinged doors is still classified by the normal wardrobe rules in section 2 / 2a (e.g. "Sliding Door Wardrobe (3-door)"), not as PAX and not as per-hole.
  • DO NOT use per-hole pricing for any FREESTANDING unit that just happens to have vertical posts or sliding doors. In particular, the following are NOT per-hole — they are normal flat-pack furniture with their own catalog names:
      – IKEA PAX wardrobe (incl. corner / L-shape, incl. mixed open + doored bays) → per-frame PAX items (see section 2b)
    ...
    Telltale sign: if the unit has its OWN feet / base frame / enclosed box carcass and could be moved without unscrewing rails and shelves off the wall, it is NOT per-hole.
    ...
    try {
      ...
      const quote = await storage.createQuote(
        input.customer,
        {
          ...
          volumetricFee: serverPricing.volumetricFee.toFixed(2),
          total: grandTotal.toFixed(2),
          depositAmount,
          finalAmount,
          status: "submitted",
